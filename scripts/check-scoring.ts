...
console.log("quoteFound long partial (60% chunks):", quoteFound("RenderDoc과 Nsight Graphics로 프레임 병목을 분석해 드로우콜 배칭과 디스크립터 캐싱으로 평균 프레임 시간 20% 단축 그리고 완전히 지어낸 문장이 뒤에 붙음", resume));
...
// prompts build
console.log("prompt sizes", managerPrompt("jd", "", "").length, basicPrompt(view, resume).length, storyPrompt(view, basic, resume).length, targetPrompt(view, basic, story, "pending", resume).length, reasonPrompt(view, basic, story, "hold", 90, ["a"]).length);
// empty items edge
